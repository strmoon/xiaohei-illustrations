...
生图通常需要 **1 到 10 分钟**。保持任务和进程活跃，以不超过 60 秒的间隔持续等待；不要因暂时只有请求日志或尚未见到文件就断开或报错。
...
- 预计耗时 **1 到 10 分钟**；持续等待，不要主动终止生成进程。
- 命令结束后，用 `ls`、`find` 或等效检查确认目标 PNG 已存在且非空；未落盘时稍候复查。
- 确认文件后再检查和交付，未经验证不得声称已生成。
